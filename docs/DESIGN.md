...
Остаётся на усмотрение при реализации: `minRows` / `maxRows` (сейчас в messenger `max-h-32`), package manager (npm workspaces — есть `package-lock.json`).
...
| Strikethrough | `~~text~~` | — | **v2** |
| Mention `@user` | — | — | **v2** (кастомный node) |
| Emoji picker | — | — | **v2** |
...
Текущий `src/` — **источник для рефакторинга**, не финальная структура.
...
- [ ] Удалить Go (`main.go`, `go.mod`)
- [ ] Настроить monorepo (npm workspaces)
- [ ] Вынести пакет `packages/rich-editor` (`@rinsvent/rich-editor`), очистить от messenger-кода
- [x] Зафиксировать public API в `DESIGN.md`

### Фаза 1 — Core MVP
- [ ] `RichTextEditor` — рефакторинг `MessageEditor` без messenger props
- [ ] `RichTextViewer` — рефакторинг `MessageContent`
- [ ] `features` prop: bold, italic, code, quote
- [ ] HTML export/import, sanitize, normalize
- [ ] Markdown shortcuts для включённых features
- [ ] `ref` handle: getHtml, clear, focus
- [ ] Базовые стили (CSS variables)

### Фаза 2 — Расширенное форматирование
- [ ] Lists (ul/ol)
- [ ] Links (auto-link + markdown)
- [ ] Code blocks + optional highlight.js в viewer
- [ ] Markdown paste plugin (feature flag)
- [ ] `enterBehavior` (submit / newline / shift-newline)

### Фаза 3 — Слоты и кастомизация
- [ ] Compound components: `ToolbarStart`, `ToolbarEnd`, `ToolbarMenu`, `SubmitButton`, `Footer`
- [ ] `useRichTextEditor()` hook + `EditorContext`
- [ ] `labels` prop для i18n toolbar
- [ ] `value` prop (внешняя подмена), `onBlur`, disabled state

### Фаза 4 — Demo + DX
- [ ] Demo app со всеми страницами
- [ ] README: install, quick start, API reference
- [ ] Сборка ESM + CJS + types (`tsup` / `vite build lib`)
- [ ] Tree-shakeable exports

### Фаза 5 — Полировка (после первого внедрения)
- [ ] Strikethrough
- [ ] Mentions (`@user`) — кастомный Lexical node
- [ ] Темы оформления (presets)
- [ ] SSR-safe viewer (без `document` на сервере — уже частично есть)
- [ ] Unit-тесты: html normalize, sanitize, markdown transformers
- [ ] a11y: aria на toolbar, keyboard shortcuts
...
| 2026-06-27 | Зафиксированы решения: `@rinsvent/rich-editor`, compound slots, value без onChange, Vite demo |
